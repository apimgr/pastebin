...
- URL shortening: `is_link` is auto-detected from content, never a client-settable flag — a paste becomes a link when its entire (trimmed) content is exactly one absolute `http://`/`https://` URL and nothing else (no extra whitespace, no additional lines, no surrounding text); no checkbox/form field/JSON field/header controls it; auto-generated short ID only (no vanity/custom slugs); visiting the short URL (`GET /{id}`, web and API) issues a `302 Found` redirect to the target — target is never fetched or validated server-side beyond scheme/format, so no SSRF exposure; if no title is supplied, a link paste's title defaults to the target URL itself, not the generic "Untitled" fallback; `/raw`, `/{id}/raw`, `/view/raw/{id}`, `/view/{id}`, and `/dl/{id}` never redirect regardless of `is_link` — they always render/return the literal stored content, so the target URL is always retrievable as text or viewable as a normal paste page; shares the paste table, expiry, burn-after, visibility, owner-token deletion, and view-count tracking unchanged; `language`/syntax highlighting do not apply to links
