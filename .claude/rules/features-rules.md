...
- 11 required tasks: `ssl_renewal`, `geoip_update`, `blocklist_update`, `cve_update`,
  `update_check`, `token_cleanup`, `log_rotation`, `backup_daily`, `backup_hourly`,
...
- Filename format: manual/timestamped `{name}_backup_YYYY-MM-DD_HHMMSS.tar.gz[.enc]`; scheduled daily fulls `{name}_backup_YYYY-MM-DD.tar.gz[.enc]`
- SHA-256 manifest included in every backup
- Encryption: AES-256-GCM + Argon2id key derivation (when password set)
- Restore: verify checksum, decrypt test, manifest, format before extracting — abort on any verification failure
- NEVER store backup password in plaintext

## Update Command (PART 22)
- `CheckForUpdate`: GitHub release API; 404 = no update available
- `DoUpdate`: download to temp file → SHA-256 verify against release `checksums.txt` → atomic `os.Rename` onto binary path
