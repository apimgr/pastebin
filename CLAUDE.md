...
## File Locations
- Config: `/etc/apimgr/pastebin/server.yml` (root) or `~/.config/apimgr/pastebin/server.yml` (user)
- Data: `/var/lib/apimgr/pastebin/` (root) or `~/.local/share/apimgr/pastebin/` (user)
- Logs: `/var/log/apimgr/pastebin/` (root) or `~/.local/log/apimgr/pastebin/` (user)
- Source: `src/`
- Docker: `docker/`
